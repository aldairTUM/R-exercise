...
```{r}
students <- read.csv("data/student-mat.csv", sep = ",")
str(students)
...
students %>%
  ...
  stat_qq(distribution = qpois, dparams = list(lambda = mean(students$G1)), color = "steelblue") +
  facet_wrap(~ Exam) +
  theme_minimal()
```
...
glm_model1 <- glm(G1 ~ ., data = students, family = gaussian())
summary(glm_model1)
```
...
# Histograms
par(mfrow = c(1, 2))
hist(pearson_resid, main = "Pearson Residuals", col = "lightblue")
hist(deviance_resid, main = "Deviance Residuals", col = "salmon")
par(mfrow = c(1, 1))

# Normality check
qqnorm(pearson_resid)
qqline(pearson_resid, col = "blue")
